...
async function createDeck(name, items) {
  const v = normalizeDeck(name, items);
  if (v.error) return v;
  const now = Date.now();
  for (let i = 0; i < 8; i++) {
    const code = dcode();
    const deck = { code, name: v.name, odai: v.odai, createdAt: now, updatedAt: now };
    try {
      if (await writeDeck(code, deck, true)) return deck;
    } catch (e) { return { error: '通信エラーが発生しました' }; }
  }
  return { error: '作成に失敗しました。もう一度お試しください' };
}

// 同じコードのまま中身を差し替える。作成済みのルームは保存時点の内容を
// 抱えているので、進行中のゲームには影響しない
async function updateDeck(code, name, items) {
  const cur = await getDeck(code);
  if (cur.error) return cur;
  const v = normalizeDeck(name, items);
  if (v.error) return v;
  const deck = { code: cur.code, name: v.name, odai: v.odai, createdAt: cur.createdAt, updatedAt: Date.now() };
  try {
    await writeDeck(cur.code, deck, false);
  } catch (e) { return { error: '通信エラーが発生しました' }; }
  return deck;
}

// ===== Room CRUD =====
async function createRoom(hostName, totalRounds, answerSeconds, revealSeconds, deckCode) {
  const hostId = rid();
...
module.exports = {
  ...
  listOdai, createOdai, updateOdai, deleteOdai, resetOdai,
  getDeck, createDeck, updateDeck, getSharedOdai: getOdaiList,
};
