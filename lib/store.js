...
// ===== Room CRUD =====
function createRoom(hostName, totalRounds) {
  cleanup();
  let code;
  ...
  const room = {
    ...
    round: 0, oyaIdx: 0,
    odai: null, oyaAnswer: [], predictions: {}, submitted: {},
    roundResults: [], usedOdaiIds: [], topicShownAt: null,
  };
  rooms.set(code, room);
...
function processAction(code, playerId, action, data) {
  ...
  switch (action) {
    ...
    case 'submit_oya': {
      if (room.phase !== 'oya_select') return { error: 'フェーズ違い' };
      ...
      room.oyaAnswer = data.answer;
      room.phase = 'predicting';
      room.submitted = {};
      break;
    }
    case 'submit_predict': {
      if (room.phase !== 'predicting') return { error: 'フェーズ違い' };
      if (room.players[room.oyaIdx].id === playerId) return { error: '親は予想不可' };
      if (room.submitted[playerId]) return { error: '提出済み' };
      if (!Array.isArray(data.answer) || data.answer.length !== 3) return { error: '3つ選んでください' };
      room.predictions[playerId] = data.answer;
      room.submitted[playerId] = true;
      const oya = room.players[room.oyaIdx];
      if (room.players.filter(p => p.id !== oya.id).every(p => room.submitted[p.id])) {
        calcResults(room);
        room.phase = 'results';
      }
      break;
...
function startRound(room) {
  room.round++;
  room.predictions = {}; room.submitted = {};
  room.oyaAnswer = []; room.roundResults = [];
  const avail = ODAI.filter(o => !room.usedOdaiIds.includes(o.id));
  if (!avail.length || room.round > room.totalRounds) { room.phase = 'final'; return; }
  const odai = avail[Math.floor(Math.random() * avail.length)];
  room.usedOdaiIds.push(odai.id);
  room.odai = odai;
  room.phase = 'topic';
  room.topicShownAt = Date.now();
}
...
function calcResults(room) {
  ...
  room.players.forEach(p => {
    ...
    const pred = room.predictions[p.id];
    if (!pred || pred.length !== 3) {
      room.roundResults.push({id:p.id,name:p.name,pred:[],yaku:'未提出',pts:0,cls:'hazure'});
      return;
    }
    const r = calcScore(room.oyaAnswer, pred);
    p.score += r.pts;
    room.roundResults.push({id:p.id,name:p.name,pred,...r});
  });
...
function buildView(room, playerId) {
  // Auto-advance topic → oya_select
  if (room.phase === 'topic' && room.topicShownAt && Date.now() - room.topicShownAt > 4000) {
    room.phase = 'oya_select';
  }
  const oya = room.players[room.oyaIdx] || {};
  const v = {
    phase: room.phase,
  ...
  if (['topic','oya_select','predicting','results'].includes(room.phase)) v.odai = room.odai;
  if (room.phase === 'predicting') {
    v.submittedStatus = {};
    room.players.forEach(p => { if (p.id !== oya.id) v.submittedStatus[p.id] = !!room.submitted[p.id]; });
  }
  if (room.phase === 'results' || room.phase === 'final') {
    v.oyaAnswer = room.oyaAnswer;
    v.predictions = room.predictions;
    v.roundResults = room.roundResults;
    if (room.phase === 'results') v.odai = room.odai;
  }
  if (room.phase === 'final') {
